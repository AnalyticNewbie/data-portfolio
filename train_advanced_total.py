...
def train():
    ...
    # Query must match the Win Model features exactly + Target (Total)
    sql = """
        SELECT 
            g.game_id, 
    ...
    print("Training Advanced Total Model...")
    pipe.fit(X_train, y_train)
    
    mae = mean_absolute_error(y_test, pipe.predict(X_test))
    print(f"Total Points MAE: {mae:.2f} points")
    
    os.makedirs("models", exist_ok=True)
    joblib.dump(pipe, "models/total_model.joblib")
    print("Saved models/total_model.joblib")
